\begin{titlepage}
  \thispagestyle{empty}
  %--------------- COMANDO LINEA----------------->
  \newcommand{\linea}{\rule{\linewidth}{0.5mm}}                 
  \center
  %<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

  %Add logos
  \includegraphics[width=0.24\textwidth]{~/unam_logo.png} \hspace{1.5cm}
  \includegraphics[width=0.25\textwidth]{~/fc_logo.png} \\[1cm]
  \textbf{\Large UNIVERSIDAD NACIONAL AUTÓNOMA}\\[3mm]
  \textbf{\Large DE MÉXICO} \\[6mm]
  \textit{\Large Facultad de Ciencias}

  \vfill

  %---------------TÍTULO----------------->
  \linea
  \vfill \vspace{1cm}
  \textbf{\LARGE Sistemas Operativos}\\[0.7cm]
  \textbf{\Large Fundamentos de Procesamiento Paralelo\\[0.4cm] y Arquitectura RP2040}\\[0.5cm]
  \linea \\
  \vfill
  \vspace{0.8cm}
  %Title of the Research
  \textbf{\Large  Práctica 10}\\[6mm]
  %<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

  %Team
  \vfill
  \textit{\small Presenta:}\\
  \textbf{\large \textbf{Contreras Morales Román Ismael: 1}}\\[2mm]
  \textbf{\large \textbf{Hernńandez Flores Karina: 3}}\\[2mm]
  \textbf{\large \textbf{Rojo Peña Manuel Ianluck: 2}}\\[3mm]

  %Profesor y Grupo
  \textit{\small Profesor:}\\
  \textbf{Javier León Cotonieto}\\[3mm]
  \textit{\small Ayudante laboratorio:}\\
  \textbf{Adrián Martínez Manzo}\\[3mm]
  \small Grupo: 7004, 2026-2\\[3mm]
  \vfill

  %Fecha or Date
  \textit{\small Fecha de entrega:}\\
         {\large 25 de mayo, 2026}
         
\end{titlepage}
