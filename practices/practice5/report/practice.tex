...
sys_semaphore_wait(semaphore_t *sem)
{
  ...
  if (sem->count > 0) {
      //Resource available - take it and continue
      sem->count--;
      __asm volatile ("CPSIE i");
      return;
  }

  //No resources available - block the current task
  ...
  //Re-enable interrupts before triggering PendSV
  __asm volatile ("CPSIE i");
...
sys_semaphore_post(semaphore_t *sem)
{
  ...
  if (sem->size > 0) {
      ...
      //count is NOT incremented - the resource passes hand-to-hand
      tasks[woken_id].state = READY;
  } else {
      //Nobody waiting - return the resource to the semaphore
      sem->count++;
  }
...
schedule(uint32_t current_sp)
{
  if (current_task != -1 && tasks[current_task].state == RUNNING) {
      ...
      tasks[current_task].state = READY;   // return to ready queue
  } else if (current_task != -1 && tasks[current_task].state == BLOCKED) {
      // Save SP but do NOT change state - task stays BLOCKED
      tasks[current_task].sp = (uint32_t *)current_sp;
  }

  //Round Robin: skip DORMANT and BLOCKED tasks
  int next_task = current_task;
  int tries     = 0;
  do {
      next_task = (next_task + 1) % MAX_TASKS;
      tries++;
      if (tasks[next_task].state == READY) break;
