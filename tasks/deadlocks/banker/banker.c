...
/* ── Internal helper: vector comparison ─────────────────────────────────── */
 ...
 */
static bool vec_leq(const int *a, const int *b, int len)
{
    for (int j = 0; j < len; j++)
        if (a[j] > b[j])
            return false;
 ...
 */
BankerStatus request_resources(BankerState *state, int thread_id,
                                const int request[])
{
 ...
 */
BankerStatus release_resources(BankerState *state, int thread_id,
                                const int release[])
{
