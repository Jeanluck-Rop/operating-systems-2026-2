 ...
 * main.c
 *
 * Banker's Algorithm — demonstration with the textbook exercise:
 *
 ...
 */
static void init_state(BankerState *s)
{
    s->n_threads   = 5;
    s->n_resources = 4;
    ...
    /* Available */
    int avail[] = {2, 2, 2, 4};
    memcpy(s->available, avail, sizeof(avail));
    ...
    for (int i = 0; i < s->n_threads; i++)
        memcpy(s->allocation[i], alloc[i], sizeof(alloc[i]));
    ...
    for (int i = 0; i < s->n_threads; i++)
        memcpy(s->max[i], max[i], sizeof(max[i]));
 ...
 */
static void check_request(const BankerState *initial,
                           int thread_id, const int *request,
                           const char *label)
{
